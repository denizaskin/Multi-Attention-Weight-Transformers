...
device = torch.device("cpu")  # Force CPU to avoid MPS deadlock
use_fp16 = False  # Disable FP16 since it's not needed on CPU
...
class GRPODepthSelector(nn.Module):
    def __init__(self, depth_dim, hidden_dim=128, use_grpo=False):
        ...
        self.use_grpo = use_grpo  # Store as instance variable
...
class ModifiedMistralAttention(nn.Module):
    def __init__(self, original_attention, num_heads=32, head_dim=128, depth_dim=8, use_grpo=False, user_depth_index=4):
        super().__init__()
        self.num_heads = num_heads
        self.head_dim = head_dim
        self.depth_dim = depth_dim
        self.depth_selector = GRPODepthSelector(depth_dim, use_grpo=use_grpo)  # Pass use_grpo
        ...
        self.user_depth_index = user_depth_index  # Store as instance variable
    ...
    def forward(self, Q, K, V):
        ...
        if self.training:
            ...
            selected_V = transformed_values[..., self.user_depth_index]  # Use self.user_depth_index
            selected_V = selected_V.permute(0, 2, 1, 3)  # (B, num_heads, seq_len, head_dim)
            attn_weights = attn_5d[..., self.user_depth_index]  # Use self.user_depth_index
...
def load_model(model_name, use_fp16):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        model_name,
        dtype=torch.float16 if use_fp16 else torch.float32,  # Changed from torch_dtype to dtype
...
def modify_model_attention(model, use_grpo=False, user_depth_index=4):  # Added user_depth_index parameter
    ...
    for name, module in target_modules:
        if name == last_layer_name:
            ...
            new_module = ModifiedMistralAttention(module, use_grpo=use_grpo, user_depth_index=user_depth_index)  # Pass user_depth_index
...
if __name__ == "__main__":
    ...
    model, last_layer_index = modify_model_attention(model, use_grpo=use_grpo, user_depth_index=user_depth_index)  # Pass user_depth_index
